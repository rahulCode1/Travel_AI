...
const MyTrips = ({ trips }) => {
  return (
    <main className={`${styles.wrapper} bg-slate-900 h-screen`}>
    

      <div className="w-11/12 mx-auto py-3 bg-slate-900">
        ...
        {trips && trips.length > 0 ? (
          trips.map((trip, i) => (
            <Link key={trip.id} to={`${trip.id}`} className={styles.tripCard}>
              ...
              <span
                className={`${styles.badge} ${trip.isTripComplete ? styles.done : styles.wip}`}
              >
